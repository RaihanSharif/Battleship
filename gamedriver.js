...
function initBoards() {
  ...
  computerPlayer.board.placeShip(6, 6, "horizontal", 1);
}
...
computerPlayerContainer.addEventListener("click", (event) => {
  if (event.target.classList.contains("boardCell")) {
    ...
    const attackResult = humanPlayer.board.receiveAttack(x, y);
    console.log(attackResult);
    changeCellStatus(event.target, attackResult);
    console.log(humanPlayer.board.missCoords);
    console.log(humanPlayer.board.hitCoords);
  }
});
